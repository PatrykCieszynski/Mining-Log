...
def detect_radar_from_game(game_image, radar_template_path, threshold=0.8, scale_range=(0.65, 1.35), scale_step=0.01, debug=False):
    ...
    if radar_template is None:
        raise FileNotFoundError(f"Failed to load radar template: {radar_template_path}")
    ...
    if best_match_val < threshold:
        raise ValueError(f"Failed to detect radar (best match: {best_match_val}).")
...
def detect_coords_from_radar(game_image, coords_template_path, radar_top_left, radar_bottom_right, scale, debug=False):
    ...
    if coords_template is None:
        raise FileNotFoundError(f"Failed to load coordinates template: {coords_template_path}")
    ...
    if max_val < 0.75:
        raise ValueError(f"Failed to detect coordinates (best match: {max_val}).")
...
def extract_coordinates_from_ocr(ocr_result):
    ...
    if lon_match and lat_match:
        ...
        raise ValueError("Failed to extract Lon/Lat coordinates from radar.")
